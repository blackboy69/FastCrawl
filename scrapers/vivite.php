...
class vivite extends baseScrape
{
...
/*	
	...
	*/		
		...
		$geos = db::query("SELECT zip, max(lat) as lat ,max(lon) as lon from geo.locations where pop>30000 group by zip",true);
		$urls = array();
		foreach($geos as $geo)
		{
			$zip = $geo['zip'];
		...
		$productIdMap[10] = 'viviteskincare';*/
			...
			$urls[]="http://www.botoxcosmetic.com/App_Controls/FindADoc/WebServices/ws_DocList.ashx?r=2.2&SearchType=1&CenterLatitude=$lat&CenterLongitude=$lon&Radius=50&ProductID=10&ZipCode=$zip&SortBy=0&showPictureID=true";
