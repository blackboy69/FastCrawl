...
class lawagencyonline extends baseScrape
{
   ...
   public function runLoader()
   {
	...
	//	$this->proxy='127.0.0.1:9996';
		...
		//db::query("DELETE FROM raw_data where type='lawagencyonline'   ");
		//db::query("DELETE FROM load_queue where type='lawagencyonline'   ");
	//	db::query("DROP TABLE lawagencyonline");
		//db::query("UPDATE  raw_data set parsed=0 where type='$type' and parsed=1");
		
		$urls = file("lawagencyonline.txt");
		//	$urls = array;
	...
	public static function parse($url,$html,$break=false)
	{
		...
		{		
			$urls = array();
			foreach ($x->query("//a[contains(translate(text(), 'CONTACT', 'contact'),'contact') or contains(translate(text(), 'ABOUT', 'about'),'about')]") as $node)
			{
