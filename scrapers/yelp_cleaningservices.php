...
class yelp_cleaningservices extends baseScrape
{
...
//	$this->debug=true;
	//log::error_level(ERROR_DEBUG_VERBOSE);
	   ...
	   db::query("
			DELETE FROM raw_data WHERE type ='yelp_cleaningservices' and LENGTH(html) < 3000
		");
	   db::query("
		UPDATE load_queue
		SET PROCESSING=0		 
		 WHERE			 
  	       url NOT IN (SELECT url FROM raw_data WHERE type ='yelp_cleaningservices')
			 AND
			 PROCESSING=1
			 ");
		db::query("UPDATE raw_data set parsed = 0 where type='yelp_cleaningservices' and parsed = 1  ");
*/
