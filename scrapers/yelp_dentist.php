...
class yelp_dentist extends baseScrape
{
...
/*		 db::query("		delete from raw_data WHERE type ='yelp_dentist' and url like '%captcha%' or url like '%adredir?%'");
	   ...
	   db::query("
		delete from load_queue
		 ...
		 WHERE			 
			 (
			 url IN (SELECT url FROM raw_data WHERE type ='yelp_dentist' and ( LENGTH(html) < 3000) or url like '%captcha%')
			 AND type ='yelp_dentist'
			 AND processing = 1
			 )
			 OR
			 (
			 ...
			 OR 
			 (
  			     url NOT IN (SELECT url FROM raw_data WHERE type ='yelp_dentist')
				  AND processing = 0
			     AND type ='yelp_dentist'
		    )
		
			 ");
		db::query("UPDATE raw_data set parsed = 0 where type='yelp_dentist' and parsed = 1  ");
*/
		...
		$this->loadUrlsByLocation("http://www.yelp.com/search?find_desc=Dentist&find_loc=%CITY%,%STATE%&ns=1&rpp=40")	;
