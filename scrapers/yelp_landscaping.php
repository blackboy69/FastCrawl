...
class yelp_landscaping extends baseScrape
{
...
//	$this->debug=true;
	//log::error_level(ERROR_DEBUG_VERBOSE);
	   ...
	   db::query("
			DELETE FROM raw_data WHERE type ='yelp_landscaping' and LENGTH(html) < 3000
		");
	   db::query("
		UPDATE load_queue
		SET PROCESSING=0		 
		 WHERE			 
  	       url NOT IN (SELECT url FROM raw_data WHERE type ='yelp_landscaping')
			 AND
			 PROCESSING=1
			 ");
		db::query("UPDATE raw_data set parsed = 0 where type='yelp_landscaping' and parsed = 1  ");
*/
		...
		$this->loadUrlsByCity("http://www.yelp.com/search?find_desc=Landscaping&find_loc=%CITY%,%STATE%&ns=1&rpp=40",'',100)	;
		$this->loadUrlsByCity("http://www.yelp.com/search?find_desc=Lawn+Care&find_loc=%CITY%,%STATE%&ns=1&rpp=40",'',100)	;
	}
