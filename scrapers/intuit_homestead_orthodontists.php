...
class intuit_homestead_orthodontists extends intuit_homestead_base
{
...
//	$this->debug=true;
	//log::error_level(ERROR_DEBUG_VERBOSE);
	   ...
	   db::query("
		delete from load_queue
		 ...
		 WHERE			 
			 (
			 url IN (SELECT url FROM raw_data WHERE type ='intuit_homestead_orthodontists' and LENGTH(html) < 3000)
			 AND type ='intuit_homestead_orthodontists'
			 AND processing = 1
			 )
			 OR
			 (
			 ...
			 OR 
			 (
  			     url NOT IN (SELECT url FROM raw_data WHERE type ='intuit_homestead_orthodontists')
				  AND processing = 0
			     AND type ='intuit_homestead_orthodontists'
		    )
	...
	//	db::query("UPDATE raw_data set parsed = 0 where type='intuit_homestead_orthodontists' and parsed = 1  ");
...
//		$this->switchProxy(null,true);
		...
		$this->loadUrl("http://business.intuit.com/directory/c135-orthodontists");	
...
$r= new intuit_homestead_orthodontists();
$r->parseCommandLine();
