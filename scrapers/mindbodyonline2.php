...
class mindbodyonline2 extends baseScrape
{
...
foreach($diff1 as $row)
{
	...
	foreach(db::query("SELECT * FROM mindbodyonline where NAME = '$name' and city='$city'",true) as $data)
	{
//		print_r($data);
...
//				$d = db::query("SELECT * FROM $table where id = $id");
	...
	function parse($url,$html)
	{
		...
		$data = db::query("SELECT * FROM mindbodyonline where id = $id ");
		if (! empty($data))
		{
			$ydata = $y->parse($html);
			if (!empty($ydata))
