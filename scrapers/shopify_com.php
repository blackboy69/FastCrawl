...
class shopify_com extends baseScrape
{
...
//		$urls[] = self::$bing->url("site:lightspeedwebstore.com");
		...
		while ($r = mysql_fetch_row($result))
      {
			...
			$urls[] = self::$bing->url("site:myshopify.com $city");

			
		 //  $urls[] = self::$google->url("site:sites.shopify.com  $city, $state");
		}
//		db::query("UPDATE raw_data set parsed = 0 where type='$type' and parsed = 1 ");
		
		foreach (range('A', 'Z') as $letter) {
		//	$this->loadUrl("http://byronwhitlock.com/fastcrawl/casper.php?type=render&&&p1=".urlencode(self::$bing->url("site:sites.shopify.com $letter",true)),true);
		}
//		$urls = array("https://traviscountystrength.sites.shopify.com/location.cfm");
//		$urls = array("https://cfemc.sites.shopify.com/location.cfm");
		$this->loadUrlsByArray($urls);
					
		 //grab the recon-ng supplemental data
		// $this->loadUrlsByArray(file("$type.txt"));

		$this->loadUrl("http://byronwhitlock.com/fastcrawl/casper.php?type=render&&p1=".urlencode(self::$bing->url("site:myshopify.com",true)),true);
		...
		if (preg_match("/bing|google/",$host))
		{
			//if (empty($query['start']) || $query['start'] > 500) // only the first 50 pages
			{
				...
				foreach ($hrefs as $href)
				{
					...
					if (preg_match("/shopify.com/",$hrefHost))
					{						
...
/*					else 
		...
		else if (preg_match("#shopify.com#",$host))
		{
			$data = array();
			$x = new  XPath($html);	
