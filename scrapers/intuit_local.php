...
class intuit_local extends baseScrape
{
...
//		
		...
		$webRequests= array();
		//db::query("DROP TABLE intuit_local");
		//db::query("DELETE FROM load_queue where type='$type'");
		//db::query("DELETE FROM raw_data where type='$type'");
		//db::query("UPDATE raw_data set parsed = 0 where type='intuit_local' and parsed = 1   ");
		//db::query("UPDATE raw_data set parsed = 0 where type='intuit_local' and parsed = 1 and url not like '%search%'   ");
		//db::query("UPDATE raw_data set parsed = 0 where type='intuit_local' and parsed = 1 and url like '%portland%'  ");

		$this->loadUrlsByLocation("http://local.intuit.com/businesses?q=&p=1&lc=%LAT%%2C%LON%&ls=0.17367689898779304%2C0.2197265625&lz=11",null,5000);	
...
/*
	...
	public static function parse($url,$html)
	{
		...
		$links = array();
		$data = array();
		if (preg_match("#local.intuit.com/businesses#",$url))
		{
