...
class intuit_homestead_dermatologists extends intuit_homestead_base
{
	...
	//log::error_level(ERROR_DEBUG_VERBOSE);
	   ...
	   db::query("
		delete from load_queue
		 ...
		 WHERE			 
			 (
			 url IN (SELECT url FROM raw_data WHERE type ='intuit_homestead_dermatologists' and LENGTH(html) < 3000)
			 AND type ='intuit_homestead_dermatologists'
			 AND processing = 1
			 )
			 OR
			 (
			 ...
			 OR 
			 (
  			     url NOT IN (SELECT url FROM raw_data WHERE type ='intuit_homestead_dermatologists')
				  AND processing = 0
			     AND type ='intuit_homestead_dermatologists'
		    )
		...
		*/
		...
		db::query("UPDATE raw_data set parsed = 0 where type='intuit_homestead_dermatologists' and parsed = 1  ");
...
//		$this->switchProxy(null,true);
		...
		$this->loadUrl("http://business.intuit.com/directory/c136-dermatologists");	
...
$r= new intuit_homestead_dermatologists();
$r->parseCommandLine();
