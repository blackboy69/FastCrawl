...
class yelp_spas extends baseScrape
{
	...
	//log::error_level(ERROR_DEBUG_VERBOSE);
	   ...
	   db::query("
		delete from load_queue
		 ...
		 WHERE			 
			 (
			 url IN (SELECT url FROM raw_data WHERE type ='yelp_spas' and LENGTH(html) < 3000)
			 AND type ='yelp_spas'
			 AND processing = 1
			 )
			 OR
			 ...
			 OR 
			 (
  			     url NOT IN (SELECT url FROM raw_data WHERE type ='yelp_spas')
				  AND processing = 0
			     AND type ='yelp_spas'
		    )
		...
		*/
		...
		$this->loadUrlsByCity("http://www.yelp.com/search?find_desc=Beauty+and+Spas&find_loc=%CITY%,%STATE%&ns=1&rpp=40");
