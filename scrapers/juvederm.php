...
class juvederm extends baseScrape
{
...
*/	
		...
		$geos = db::query("SELECT zip, max(lat) as lat ,max(lon) as lon from geo.locations where pop>40000 group by zip",true);
		$urls = array();
		foreach($geos as $geo)
		{
			$zip = $geo['zip'];
			...
			$urls[]="http://www.botoxcosmetic.com/App_Controls/FindADoc/WebServices/ws_DocList.ashx?r=2.2&SearchType=1&CenterLatitude=$lat&CenterLongitude=$lon&Radius=50&ProductID=2&ZipCode=$zip&SortBy=0&showPictureID=true";
