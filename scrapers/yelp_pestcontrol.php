...
class yelp_pestcontrol extends baseScrape
{
...
//	$this->debug=true;
	//log::error_level(ERROR_DEBUG_VERBOSE);
	   ...
	   db::query("
			DELETE FROM raw_data WHERE type ='yelp_pestcontrol' and LENGTH(html) < 3000
		");
	   db::query("
		UPDATE load_queue
		SET PROCESSING=0		 
		 WHERE			 
  	       url NOT IN (SELECT url FROM raw_data WHERE type ='yelp_pestcontrol')
			 AND
			 PROCESSING=1
			 ");
		db::query("UPDATE raw_data set parsed = 0 where type='yelp_pestcontrol' and parsed = 1  ");
*/
		...
		$this->loadUrlsByCity("http://www.yelp.com/search?find_desc=Pest+Control&find_loc=%CITY%,%STATE%&ns=1&rpp=40",'',100)	;
	}
