...
class intuit_homestead_contractors extends intuit_homestead_base
{
...
//	$this->debug=true;
	//log::error_level(ERROR_DEBUG_VERBOSE);
	   ...
	   db::query("
		delete from load_queue
		 ...
		 WHERE			 
			 (
			 url IN (SELECT url FROM raw_data WHERE type ='intuit_homestead_contractors' and LENGTH(html) < 3000)
			 AND type ='intuit_homestead_contractors'
			 AND processing = 1
			 )
			 OR
			 (
			 ...
			 OR 
			 (
  			     url NOT IN (SELECT url FROM raw_data WHERE type ='intuit_homestead_contractors')
				  AND processing = 0
			     AND type ='intuit_homestead_contractors'
		    )
	...
	//	db::query("UPDATE raw_data set parsed = 0 where type='intuit_homestead_contractors' and parsed = 1  ");
...
//		$this->switchProxy(null,true);
		...
		$this->loadUrl("http://business.intuit.com/directory/c36-contractors-construction");	
...
$r= new intuit_homestead_contractors();
$r->parseCommandLine();
