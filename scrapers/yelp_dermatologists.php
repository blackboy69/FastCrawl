...
class yelp_dermatologists extends baseScrape
{
   ...
   public function runLoader()
   {
	   ...
	   db::query("
		delete from load_queue
		 ...
		 WHERE			 
			 (
			 url IN (SELECT url FROM raw_data WHERE type ='yelp_dermatologists' and LENGTH(html) < 3000)
			 AND type ='yelp_dermatologists'
			 AND processing = 1
			 )
			 OR
			 (
			 ...
			 OR 
			 (
  			     url NOT IN (SELECT url FROM raw_data WHERE type ='yelp_dermatologists')
				  AND processing = 0
			     AND type ='yelp_dermatologists'
		    )
		...
		*/
		...
		$urls = array();
		$this->loadUrlsByLocation("http://www.yelp.com/search?find_desc=Dermatologists&find_loc=%CITY%,%STATE%&ns=1&rpp=40")	;
		$this->loadUrlsByLocation("http://www.yelp.com/search?find_desc=Family+Practice&find_loc=%CITY%,%STATE%&ns=1&rpp=40")	;
		$this->loadUrlsByLocation("http://www.yelp.com/search?find_desc=Podiatrists&find_loc=%CITY%,%STATE%&ns=1&rpp=40")	;
		$this->loadUrlsByLocation("http://www.yelp.com/search?find_desc=Cosmetic+Surgeons&find_loc=%CITY%,%STATE%&ns=1&rpp=40")	;
	}
	...
	public function csvSql()
	{
		...
		return array(
				"Dermatologists" => "SELECT * from $table where CATEGORIES like '%Dermatologists%' ",
				"Family Practice" => "SELECT * from $table where CATEGORIES like '%Dermatologists%' ",
				"Cosmetic Surgeons" => "SELECT * from $table where CATEGORIES like '%Dermatologists%' ",
				"Podiatrists" => "SELECT * from $table where CATEGORIES like '%Dermatologists%' "
