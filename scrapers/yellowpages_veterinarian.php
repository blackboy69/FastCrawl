...
class yellowpages_veterinarian extends baseScrape
{
	...
	//log::error_level(ERROR_DEBUG_VERBOSE);
	   ...
	   db::query("
		UPDATE load_queue SET processing = 1 WHERE url IN (SELECT url FROM raw_data)		 
			 ");
		db::query("UPDATE raw_data set parsed = 0 where type='yellowpages_veterinarian' and parsed = 1   ");
		*/
...
//		$this->proxy = "localhost:9666";
		...
		db::query("UPDATE raw_data set parsed = 0 where type='yellowpages_veterinarian'  and parsed = 1   ");
			db::query("UPDATE load_queue SET processing = 1 WHERE  type='yellowpages_veterinarian' and url IN (SELECT url FROM raw_data where type='yellowpages_veterinarian')					 ");

		//$this->proxy = "127.0.0.1:9666";
		$this->loadUrlsByCity("http://www.yellowpages.com/search?tracks=true&search_terms=veterinarian&geo_location_terms=%CITY%,%STATE%")	;
...
$r= new yellowpages_veterinarian();
$r->parseCommandLine();
