...
//crappyt hting off stackoverflow that needed much love to deal with duplicate keys
	...
	foreach ($array as $k => $item) {
		if (is_array($item))
		{
			...
			if (sizeof($item)>1)
			{
				...
				{
					$recurseItem[$k."_".$key] = $val;
				}
			}
			$return = array_flatten($recurseItem, $return);
		}
		elseif ($item)
		{
			$return[$k] = $item;
...
function fuzzy_match($string1,$string2,$distance) {
         ...
         $lev = levenshtein(strtolower($string1), strtolower($string2));
         if ($lev <= $distance) {
